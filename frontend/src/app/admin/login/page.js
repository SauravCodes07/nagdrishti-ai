...
function AdminLoginContent() {
  ...
  return (
    <BackgroundGrid pattern="grid" className="min-h-screen flex flex-col items-center justify-center p-4 text-[#0F172A] dark:text-[#F8FAFC] antialiased">
      ...
      >
        <SpotlightCard className="w-full bg-[#FFFFFF]/95 dark:bg-[#111C2E]/95 backdrop-blur-xl border border-[#E2E8F0] dark:border-[#243244] rounded-2xl p-7 shadow-2xl space-y-5 relative overflow-hidden">
          <BorderBeam size={160} duration={9} colorFrom="#F59E0B" colorTo="#E8730A" />
          ...
          <form onSubmit={handleLogin} className="space-y-3.5">
            ...
            >
              {loading ? (
                ...
                <>
                  <span>Authenticate Command Session</span>
          ...
          <GoogleSignInButton
            requireAdmin={true}
            text="Municipal Single Sign-On"
            onSuccess={() => router.push(returnUrl)}
            onError={(msg) => setErrorMsg(msg)}
          />

          {/* Demo Credentials Note */}
          <div className="p-3 rounded-xl bg-[#F8FAFC] dark:bg-[#0B0F17] border border-[#E2E8F0] dark:border-[#243244] text-[11px] text-[#64748B] dark:text-[#94A3B8] space-y-1">
            <div className="flex items-center gap-1 font-semibold text-[#0F172A] dark:text-[#F8FAFC]">
              <ShieldCheck className="w-3.5 h-3.5 text-[#0F766E] dark:text-[#14B8A6]" />
              <span>Officer Duty Session</span>
            </div>
            <p>Access requires verified Nagpur Municipal Corporation administrative roles.</p>
          </div>
